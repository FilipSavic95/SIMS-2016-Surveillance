		...
		if (panel == 'a') {
			...
			newPanel.addMouseMotionListener(new MouseMotionAdapter() {
				...
				public void mouseDragged(MouseEvent e) {
					...
					int y = e.getY();
					/*
					 * -stratX i startY se ne mijenjaju kako bi trebalo
					 * -pomjeraj za 10
					 */
					if (startX != NONE && startY != NONE) { // znaci da je
															// PRITISNUT klik
															// misa.
															// kad se klik
															// PUSTI, start-ovi
						...
						JComponent source = (JComponent) e.getSource(); // komponenta
																		...
																		// kojom
																		// radimo
						Rectangle bounds = source.getBounds(); // granice
																// komponente
																// nad kojom
																// radimo
						int deltaX = x - startX;
						...
						if (resize) {
							...
							source.setSize(
									Math.max(10, bounds.width + x - prevX),
									Math.max(10, bounds.height + y - prevY));
						} else { // pomijeramo
							source.setLocation(bounds.x + deltaX, bounds.y
									+ deltaY);
							source.repaint();
						}
						// TODO: make sure you don't resize it as much as it
						// disappears
						// TODO: make sure you don't move it outside the main
						// panel
					} else { // znaci da JE PUSTEN (NIJE PRITISNUT) klik misa i
								// da su start-ovi NONE
						startX = x;
						startY = y;
					}
					prevX = x;
			...
			// newPanel.setMinimumSize(new Dimension(2, 2));
